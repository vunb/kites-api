...
module.exports = function (kites, definition) {
    ...
    kites.on('expressConfigure', (app) => {
        ...
        kites.db = kites.dataSource;
        kites.emit('dataSourceReady', kites.dataSource, kites);

    });

    kites.on('dataSourceReady', (dataSource, kites) => {
        ...
        _.each(kites.models, (e) => {
            ...
            e.connections = kites.db.connections;

            var Promise = require('bluebird');
            var schema = e.createSchema();

            // Promisify schema of the model
            // Object.getOwnPropertyNames(schema).forEach(function(key) {
            //     if (typeof key === 'function') {
            //         schema[key] = Promise.promisify(schema[key]);
            //     }
            // });
    ...
    kites.model = function getModel(name, useNativeModel) {
        ...
        if (options.useNativeModel) return model.schema.adapter.model(name);
